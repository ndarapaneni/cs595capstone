...
<h4> PatientsList:</h4>
<c:choose>
    <c:when test='${patientList == null || empty patientList}'>
         No patients available at this time
    </c:when>
    <c:otherwise>
      
      <table>
		<tr><td>PatientId</td><td>FirstName</td><td>LastName</td><td>PatientEmailId</td><td>PhoneNumber</td><td>City</td><td>State</td>
      <c:forEach items="${patientList}" var="patient">
<tr><td>${patient.patientId}</td><td>${patient.firstName}</td><td>${patient.lastName}</td><td>${patient.patientEmailId}</td><td>${patient.phoneNumber}</td><td>${patient.city}</td><td>${patient.state}</td>
      </c:forEach>
      </table>

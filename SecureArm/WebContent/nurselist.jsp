...
<h4> Nurse List:</h4>
<c:choose>
    <c:when test='${nurseList == null || empty nurseList}'>
         No nurses available at this time
    </c:when>
    <c:otherwise>
      
      <table>
		<tr><td>NurseId</td><td>FirstName</td><td>LastName</td><td>NurseEmailId</td><td>PhoneNumber</td><td>DeviceId</td>
      <c:forEach items="${nurseList}" var="nurse">
<tr><td>${nurse.nurseId}</td><td>${nurse.firstName}</td><td>${nurse.lastName}</td><td>${nurse.nurseEmailId}</td><td>${nurse.phoneNumber}</td><td>${nurse.deviceId}</td>
      </c:forEach>
      </table>
